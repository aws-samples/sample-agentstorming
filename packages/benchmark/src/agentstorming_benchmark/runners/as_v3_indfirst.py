...
"""AS-V3-IndependentFirst: Independent parallel answers, then conditional discussion.

Addresses both over-deliberation AND sequential contamination:
- Phase 1: All specialists answer INDEPENDENTLY in parallel (like sc_k5)
- Phase 2: Moderator synthesizes
- Phase 3: IF high disagreement, run 1 discussion round, else return synthesis

Key innovation:
- Preserves independence benefit of self-consistency
- Adds synthesis/discussion only when variance signals a hard problem
- Heterogeneous panel amplifies value of independent samples

Cost profile:
- Easy questions (consensus): 5 specialists + 1 mod = 6 calls ≈ sc_k5
- Hard questions (disagreement): 5 + 1 + 5 + 1 = 12 calls ≈ 2× sc_k5
- Expected: ~7-8 calls average (if 70% easy, 30% hard)
"""
...
# Same heterogeneous panel as AS-V2.
PANEL_V3 = {
...
def _build_prompt(q: Question) -> str:
    ...
    if q.dataset in ("mmlu_pro", "gpqa_diamond", "musr", "truthfulqa_mc", "arc_challenge", "xdomain_v4", "xdomain_v5", "musr_murder", "musr_object_placements", "musr_team_allocation"):
...
async def _run_as_v3_indfirst(question: Question, seed: int) -> RunResult:
    """AS-V3 IndependentFirst: independent parallel -> synthesis -> conditional discussion."""
    t0 = time.time()
    usage = TokenUsage()
    ...
    transcript: list[Message] = [
        Message(role="system", speaker="system",
                content="AS-V3-IndependentFirst: independent parallel answers, conditional discussion"),
    ...
    for sp, (text, u) in zip(specialists, results):
        ...
        transcript.append(Message(
            ...
            metadata={"phase": "independent", "model": PANEL_V3[sp]}
        ))
        participant_turns += 1
    ...
    transcript.append(Message(
        ...
        metadata={"phase": "synthesis", "needs_discussion": needs_discussion, "model": PANEL_V3["project-lead"]}
    ))
    moderator_turns += 1
    ...
    if should_discuss:
        ...
        for sp in specialists:
            ...
            discussion_transcript.append(Message(
                role="assistant", speaker=sp, content=text,
                metadata={"phase": "discussion", "model": PANEL_V3[sp]}
            ))
        ...
        transcript.append(Message(
            ...
            metadata={"phase": "final_synthesis", "model": PANEL_V3["project-lead"]}
        ))
        moderator_turns += 1
    ...
    # Cost approximation (same as AS-V2: use aggregate).
    total_cost = cost_usd(usage, "us.anthropic.claude-sonnet-4-6")

    return RunResult(
        config_id="as_v3_indfirst",
        dataset=question.dataset,
        question_id=question.id,
        seed=seed,
        model="panel-as-v3-indfirst",
...
@register("as_v3_indfirst")
class ASV3IndependentFirstRunner(Runner):
    """AS-V3 IndependentFirst: independent parallel -> conditional discussion."""

    async def run(self, question: Question, seed: int) -> RunResult:
        result = await _run_as_v3_indfirst(question, seed)
